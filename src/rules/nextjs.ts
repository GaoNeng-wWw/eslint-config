import type { OptionsFiles, OptionsOverrides, TypedFlatConfigItem } from '../type';
import { GLOB_SRC } from '../glob';
...
export async function nextjs(
  ...
  function getRules(name: keyof typeof pluginNextJS.configs): Record<string, any> {
    ...
    if (!rules) {
      throw new Error(`[@gaonengwww/eslint-config] Failed to find config ${name} in @next/eslint-plugin-next`);
    }
